...
const faqsRoute = () => {
    ...
    setSelectedKeywords((prevSelectedKeywords: any) => {
      ...
      if (prevSelectedKeywords.includes(selectedKeyword.name)) {
        return prevSelectedKeywords.filter(
          (keyword: any) => keyword !== selectedKeyword.name
        );
      } else {
  ...
  return (
    <View style={{ direction: isRTL ? "rtl" : "ltr", marginBottom: 30 }}>
      <View style={{ marginVertical: 16, direction: "ltr" }}>
        <FlatList
          data={faqKeywords}
          inverted={true}
          horizontal
...
const SettingsHelpCenter = () => {
  ...
  const [routes] = React.useState([
    { key: "first", title: " الأسئلة الشائعة" },
    { key: "second", title: " تواصل معنا" },
  ]);
